                    ...
                    foreach (var property in nvs.Split('.'))
                    {
                        if (scope is IList)
                        {
                            foreach (var sc in scope as IList)
                            {
                                ...
                                PropertyInfo expandProp = sc.GetType().GetProperties().SingleOrDefault(o => o.GetCustomAttribute<SerializationReferenceAttribute>()?.RedirectProperty == keyPi.Name);
                                if (expandProp != null)
                        ...
                        else
                        {
                            ...
                            if (keyPi == null)
                                continue;
                            // Get the backing property
                            PropertyInfo expandProp = scope.GetType().GetProperties().SingleOrDefault(o => o.GetCustomAttribute<SerializationReferenceAttribute>()?.RedirectProperty == keyPi.Name);
