        ...
        public IdentifiedData ResolveResource(string data)
        {
            try
            {
                ...
                if (typeof(Entity).IsAssignableFrom(type))
                {
                    ...
                    result = ApplicationServiceContext.Current.GetService<IRepositoryService<Entity>>().Find(query, 0, 2, out tr).SingleOrDefault();
                }
                else if (typeof(Act).IsAssignableFrom(type))
                {
                    var query = QueryExpressionParser.BuildLinqExpression<Act>(domainQuery);
                    result = ApplicationServiceContext.Current.GetService<IRepositoryService<Act>>().Find(query, 0, 2, out tr).SingleOrDefault();
                }
